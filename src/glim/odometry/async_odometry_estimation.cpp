...
: odometry_estimation(odometry_estimation),
  ...
  thread = std::thread([this] { run_onlyMonoslam(); });  //[TODO:phs] monoslam 을 진행하기 위한 thread 객체 생성
}
...
///[TODO:phs] 모노슬램만을 위한 Thread function
void AsyncOdometryEstimation::run_onlyMonoslam() {
  std::deque<std::pair<double, cv::Mat>> images;
  std::deque<PreprocessedFrame::Ptr> raw_frames;  ///[TODO:phs] 필요할까?? 안필요할꺼 같은데..
  while (!kill_switch) {
    auto new_images = input_image_queue.get_all_and_clear();
    images.insert(images.end(), new_images.begin(), new_images.end());  ///[TODO:phs] 만약에 SLAM 이 아니고 SFM 같이 모든 이미지를 한번에 가져오는 시스템이라면 이게 가능하다.
    if (images.empty()) {
      if (end_of_sequence) {
        break;
      }
    ...
    while (!images.empty()) {
      const std::pair<double, cv::Mat> image = images.front();
